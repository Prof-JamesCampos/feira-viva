...
| `0.9` | 12 | Documentação e organização: Swagger UI (springdoc-openapi 3.0.1) com controllers/DTOs anotados, README final do backend, `OpenApiConfig` (4 tags), **`/debug/instancias` removido** (dívida paga), seção “Padrões GoF aplicados”, convenção `/api/v1` registrada, especificação arquivada em `docs/openapi.json`, versão alinhada em `0.9.0` |
